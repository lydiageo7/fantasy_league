...
// Open the database
const db = await open({
  filename: './project2.db', // Path to your SQLite database file
...
export async function removeTeam(req, res) {
  const { teamId } = req.body; // expecting an identifier for which team to delete
...
export async function updateTeamName(req, res) {
  const { teamId, newTeamName, newNumberOfPlayers, newHomeField } = req.body; // expecting all fields to be updated

  if (!teamId || !newTeamName || !newNumberOfPlayers || !newHomeField) {
    return res.status(400).json({ error: 'Team ID, new team name, number of players and new home field are required.' });
  }
